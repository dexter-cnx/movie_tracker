...
final watchlistDataSourceProvider = Provider((ref) =>
    WatchlistLocalDataSource(Hive.box<Map>(WatchlistLocalDataSource.boxName)));
final watchlistControllerProvider =
    NotifierProvider<WatchlistController, List<WatchlistItem>>(
...
class WatchlistController extends Notifier<List<WatchlistItem>> {
  ...
  @override
  List<WatchlistItem> build() => _source.getAll();
  Future<void> save(WatchlistItem item) async {
    await _source.save(item);
    state = _source.getAll();
  }
